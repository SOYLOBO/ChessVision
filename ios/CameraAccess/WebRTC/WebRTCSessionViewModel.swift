...
class WebRTCSessionViewModel: ObservableObject {
  ...
  func startSession() async {
    ...
    // Trigger Local Network permission prompt (required by iOS 14+).
    // NWBrowser is the only reliable way to trigger the system dialog.
    triggerLocalNetworkPermission()

    // Small delay to let the permission dialog appear and be accepted
  ...
  /// Triggers the iOS Local Network permission dialog by starting a Bonjour browse.
  /// This is required before connecting to local IP addresses via WebSocket.
  private func triggerLocalNetworkPermission() {
    ...
    // Stop after a short delay — we only need it to trigger the prompt
